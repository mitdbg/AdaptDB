...
public class Query implements Serializable {
    ...
    public Query(int attr, Predicate[] predicates) {
        this.joinAttribute = attr;
        this.predicates = predicates;
    }

    public Query(String queryString) {
        String[] parts = queryString.split(";");
        this.joinAttribute = Integer.parseInt(parts[0]);
        this.predicates = new Predicate[parts.length - 1];
        for (int i = 1; i < parts.length; i++) {
            this.predicates[i - 1] = new Predicate(parts[i]);
        }
    }
    ...
    public int getJoinAttribute(){return joinAttribute;}
    ...
    public String toString() {
        String stringPredicates = String.format("%d", joinAttribute);
        if (predicates.length != 0)
            stringPredicates = stringPredicates + ";" + Joiner.on(";").join(predicates);
