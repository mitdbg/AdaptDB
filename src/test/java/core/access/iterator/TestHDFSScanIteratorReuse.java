...
public class TestHDFSScanIteratorReuse extends TestCase {
	...
	protected ReusableHDFSPartition getPartitionInstance(String path){
		return new ReusableHDFSPartition(path, propertiesFile, (short)1, buffMgr);
	}
	
	public void testScan(){
		System.out.println("Scanning: "+partitionDir+"/0");
		ReusableHDFSPartition partition = getPartitionInstance(partitionDir+"/0");
		partition.load();
...
//		hashTable.get(key).add(value);
	...
	public void testScanAll(){
		...
		for(String partitionPath: partitionPaths){
			...
			ReusableHDFSPartition partition = new ReusableHDFSPartition(partitionPath, propertiesFile, (short)1, buffMgr);			
			partition.load();
			itr.setPartition(partition);
