...
public class JoinCounts {
	...
	public void fillBucketSizes(String table) {
		...
		Path file = FileSystems.getDefault().getPath(
				"/Users/qui/Documents/exp/bucketsizes_" + table + ".csv");
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				bucketSizes.get(table).put(Integer.parseInt(tokens[0]),
	...
	public void countJoinOverlapNaive(String big, String small) {
		...
		for (int numChunks : numChunkValues) {
			...
			// Map<Integer, Integer> lineCount = new HashMap<Integer,
			// Integer>();
			// Map<Integer, Integer> partCount = new HashMap<Integer,
			// Integer>();
			for (int i = 0; i < numChunks; i++) {
				long start = domainStart + (range / numChunks) * i;
				long end = start + range / numChunks;

				Predicate part1 = new Predicate(0, TYPE.LONG, (long) start,
						PREDTYPE.GT);
				Predicate part2 = new Predicate(0, TYPE.LONG, (long) end,
						PREDTYPE.LEQ);
				...
				Predicate lineitem1 = new Predicate(1, TYPE.INT, (int) start,
						PREDTYPE.GT);
				Predicate lineitem2 = new Predicate(1, TYPE.INT, (int) end,
						PREDTYPE.LEQ);
			...
			System.out.println(numChunks + "," + lineOverlap + ","
					+ partOverlap);
		}
	}
	...
	public void testOriginalSplit(String big, String small) {
		...
		Charset charset = Charset.forName("US-ASCII");
		Path file = FileSystems.getDefault().getPath(
				"/Users/qui/Documents/exp/lineitem_1_split_combo");
		try {
			BufferedReader reader = Files.newBufferedReader(file, charset);
			String line;
			while ((line = reader.readLine()) != null) {
				String[] tokens = line.split(";");
				Predicate part1 = new Predicate(0, TYPE.LONG,
						Long.parseLong(tokens[1]), PREDTYPE.GT);
				Predicate part2 = new Predicate(0, TYPE.LONG,
				...
				for (String partitionId : tokens[0].split(",")) {
					Double size = bucketSizes.get(big).get(
