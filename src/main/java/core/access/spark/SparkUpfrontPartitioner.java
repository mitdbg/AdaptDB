...
public class SparkUpfrontPartitioner {
	...
	public void sampleBlocks(String localDataDir, final double samplingRate,
			String hdfsPath) {
		int numBlocks = (int) (1 / samplingRate * 100); // sample 100 chunks
														// from file
		SparkSamplingMapFunction f = new SparkSamplingMapFunction(samplingRate);
		JavaRDD<String> sample = ctx.textFile(localDataDir, numBlocks)
				.mapPartitions(f);
		// UUID id = UUID.randomUUID();
		// throw new RuntimeException("num "+sample.count());
		sample.coalesce(1).saveAsTextFile(
				cfg.getHADOOP_NAMENODE() + hdfsPath + "/samples/");
	}
	...
	public void createTextFile(String localDataDir, RNode newIndexTree) {
		final DistributedPartitioningIterator partitioner = new DistributedPartitioningIterator(
				cfg.getZOOKEEPER_HOSTS(), newIndexTree, propertiesFile,
				hdfsPath);
		FlatMapFunction<Iterator<String>, String> mapFunc = new SparkPartitioningMapFunction(
				partitioner);
	...
	public static void main(String[] args) {
		...
		// SparkUpfrontPartitioner p = new
		// SparkUpfrontPartitioner(propertiesFile, hdfsDir);
		// p.sampleBlocks(dataDir, samplingRate, hdfsDir);

		/*
		 * // 2. on one machine, build the index and write to hdfs. RobustTreeHs
		 * index = new RobustTreeHs(1); index.initBuild(); CartilageIndexKey key
		 * = new CartilageIndexKey('|'); InputReader r = new InputReader(index,
		 * key); FileSystem fs =
		 * HDFSUtils.getFS(HDFSUtils.getFS(ConfUtils.create(propertiesFile,
		 * "defaultHDFSPath").getHadoopHome()+"/etc/hadoop/core-site.xml");)
		 * r.scanHDFSDirectory(fs, hdfsDir + "/sample"); index.initProbe();
		 * 
		 * HDFSPartitionWriter writer = new HDFSPartitionWriter(hdfsDir,
		 * propertiesFile); byte[] indexBytes = index.marshall();
