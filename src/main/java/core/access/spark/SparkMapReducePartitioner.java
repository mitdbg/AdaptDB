...
public class SparkMapReducePartitioner extends SparkUpfrontPartitioner {
	...
	public static class MyOutputFormat extends MultipleTextOutputFormat<Integer, Iterable<String>> {
		...
		protected static class MyRecordWriter implements RecordWriter<Integer, Iterable<String>>{
	...
	public void createPartitions(String localDataDir){
		...
		//ctx.hadoopConfiguration().set(MyOutputFormat.HDFS_OUTPUT_PATH, "hdfs://localhost:9000" + hdfsPath + "/partitions");
		ctx.textFile(localDataDir)
				.mapToPair(new MyMap(t))
				.groupByKey()
				.saveAsHadoopFile(cfg.getHADOOP_NAMENODE() + hdfsPath + "/partitions", Integer.class, Iterable.class, MyOutputFormat.class);
