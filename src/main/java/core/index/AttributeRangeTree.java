...
public class AttributeRangeTree implements MDIndex {
	...
	public Object getBucketId(MDIndexKey key) {
		...
		switch (type) {
		case INT:
			val = key.getIntAttribute(attribute);
			break;
		case LONG:
			val = key.getLongAttribute(attribute);
			break;
		case DOUBLE:
			val = key.getFloatAttribute(attribute);
			break;
		case DATE:
			val = key.getDateAttribute(attribute);
			break;
		default:
	...
	public byte[] marshall() {
		return (type.toString() + ";" + String.valueOf(attribute) + ";" + Joiner
				.on(",").join(bucketBoundaries)).getBytes();
	}
	...
	public void unmarshall(byte[] bytes) {
		...
		for (int i = 0; i < boundaries.length; i++) {
			bucketBoundaries[i] = TypeUtils.deserializeValue(type,
					boundaries[i]);
		}
	}
}
