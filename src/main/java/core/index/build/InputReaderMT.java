			...
			while((nRead = ch.read(bb)) != -1){
				...
				for(int i=0; i<buffProcessors.length; i++){
					if(i<buffProcessors.length-1)
						buffProcessors[i].init(i*bytesPerThread, (i+1)*bytesPerThread);
					else
				...
				for(int i=0; i<buffProcessors.length; i++){
					...
					keys[0].setBytes(a);
					if(writer!=null)
	    				writer.writeToPartition(index.getBucketId(keys[0]).toString(), a, 0, a.length);
					//if(firstPass)
		    		//	index.insert(keys[0]);
	...
	public class BufferProcessor implements Runnable {
		...
		public void run() {
			for ( ; curr<end; curr++ ){
		    	if(byteArray[curr]==newLine){
		    		if(lineCount==0){
		    			...
		    			System.arraycopy(byteArray, previous, firstBroken, 0, firstBroken.length);
		    		}
		    		else{
		    			key.setBytes(byteArray, previous, curr-previous);
		    			if(writer!=null)
		    				writer.writeToPartition(index.getBucketId(key).toString(), byteArray, previous, curr-previous);
		    			//if(firstPass)
			    		//	index.insert(key);
		    		}
		    		previous = ++curr;
			...
			if(previous < end){
    			...
    			System.arraycopy(byteArray, previous, lastBroken, 0, lastBroken.length);
			}
		}
